...
あなたは、以下のステップを**厳密に**実行します。

**⚠️ 重要な注意事項：**
- 各ステップのチェックリストを**必ず全て**確認してください
- 1つでも欠けている場合、そのステップは不完全です
- ユーザーから指摘される前に、自分でチェックリストを確認してください
- 「対策します」と言うだけでなく、実際にチェックリストに従ってください

### Step 1: タスク受付と準備

**1. Issue取得とブランチ作成**
- ユーザーから **GitHub Issue 番号**を受け付けたらフロー開始です。
- `/create-gh-branch` カスタムコマンドを実行し、Issue の取得とブランチを作成します。

**2. 要件・設計ドキュメントの確認（必須）**

以下のドキュメントを **必ず読み、確認** してください。読まずに実装計画を策定してはいけません。

- [ ] **REQUIREMENTS.md** を読む
  - プロジェクトゴール（WHO/WHAT/HOW）を確認
  - 該当 Issue がどの主要機能（4.1節）に該当するか確認
  - UI/UX方針（5節）を確認
  - データ設計の基本方針（6節）を確認
  - セキュリティ・権限制御（7.1節）を確認
  - 成功基準（9節）を確認

- [ ] **docs/design.md** を読む
  - 該当 Issue の機能仕様（2節）を確認
  - テーブル定義・ER図（3節）を確認
  - AI機能の場合、AI機能共通仕様（4節）を確認
  - RLS仕様（5.1節）を確認

**3. 関連コードの調査（SerenaMCP必須）**

SerenaMCPを使用して、関連コードを調査します。`Read`ツールでソースファイル全体を読むことは禁止です。

- [ ] 関連するマイグレーションファイルを確認（`Glob`で検索）
- [ ] 関連するコンポーネント・関数を確認（`find_symbol`、`get_symbols_overview`）
- [ ] 既存の実装パターンを確認（`find_referencing_symbols`、`search_for_pattern`）

**Step 1完了の確認チェックリスト：**
- [ ] REQUIREMENTS.mdを読んだ
- [ ] design.mdを読んだ
- [ ] SerenaMCPで関連コードを調査した
- [ ] Issue の要件を理解した
...
以下の全ての項目を含めた実装計画を策定してください。1つでも欠けている場合、計画は不完全です。

1. **要件確認の結果**
   - REQUIREMENTS.mdのどのセクションに該当するか
   - design.mdのどのセクションに該当するか
   - プロジェクトゴール（WHO/WHAT/HOW）との整合性
   - 成功基準（実務的/技術的）との整合性
...
4. **コードレビュープロセス**
   - nextjs-reviewerサブエージェントを呼び出す
   - レビュー結果をユーザーに報告
   - **ユーザー承認を待つ（必須）**: レビュー結果報告後、必ずユーザーから承認（yes）または修正指示（fix）を受け取るまで次へ進まない
...
6. **コミット計画**
   - コミットメッセージの形式（`feat(issue-X): ...`）

**計画策定前の確認チェックリスト：**
- [ ] REQUIREMENTS.mdを読んだか
- [ ] design.mdを読んだか
- [ ] SerenaMCPで関連コードを調査したか
...
**実装計画をユーザーに提示し、承認を得ます。承認なしに次へ進んではいけません。**

### Step 3: 実装・レビュー・修正サイクル

承認された計画に基づき、以下の順序で実行します。

**1. 実装**
- [ ] TDD に従って実装（Red → Green → Refactor）
- [ ] 承認された計画の内容を全て実装

**2. テスト実行**
- [ ] `npm test` を実行し、全てパスすることを確認

**3. コードレビュー**
- [ ] `nextjs-reviewer` サブエージェントを呼び出す
- [ ] レビュー結果を確認

**4. ユーザー報告**
- [ ] 実装内容をユーザーに報告
- [ ] レビュー結果をユーザーに報告（指摘事項がある場合は詳細を報告）

**5. ユーザー承認を待つ（CRITICAL - 必ず遵守）**

⚠️ **絶対に守ること**: この時点で必ずユーザーからの指示を待ちます。ユーザーの承認なしに修正や次のステップに進むことは厳禁です。

ユーザーからの指示を受け取ったら：
  - **`yes` または 承認の意思表示**: DoD確認（Step 3-6）へ進む
  - **`fix` または 修正指示**: 以下の修正フローに従う

**修正フロー（fixの場合）**:
1. ユーザーから明示的に修正指示（`fix`等）を受け取ったことを確認
2. 指摘事項に基づき修正を実施
3. 修正完了後、Step 3-2（テスト実行）から再実行
4. Step 3-3（コードレビュー）を再実行（必要に応じて）
5. **必ず** Step 3-4（ユーザー報告）に戻り、修正内容を報告
6. 再度 Step 3-5（ユーザー承認）でユーザーの承認を待つ

**6. DoD（Definition of Done）確認**
- [ ] `npm test` が通る
- [ ] `npm run lint` が通る
- [ ] `npm run build` が成功する
...
**7. コミット**
- [ ] 変更をステージング（`git add`）
- [ ] コミットメッセージを作成（`feat(issue-X): ...`形式）
- [ ] コミット実行（`git commit`）

**Step 3完了の確認チェックリスト：**
- [ ] 全てのテストがパスした
- [ ] コードレビューを実施した
- [ ] **ユーザー承認を得た（yes/承認の意思表示を受け取った）** ← CRITICAL
- [ ] DoDを全て満たした
- [ ] コミットを完了した
