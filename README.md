...
<h1>アプリケーション概要</h1>
行った会議の音声の文字起こしを行いテキスト化したものを取り込み、生成AIを使用することで
要約・アクションプラン抽出を行い、次のアクションに繋げることを目的としたAI議事録管理アプリケーション
...
<br>※招待管理の機能はチームを作成したユーザーが使用可能</br>
...
<h1>工夫したポイント</h1>
<br>・CLAUDE.mdにワークフローを設け、作成したissueの読み取りからブランチ作成、要件・設計ドキュメントの確認、コードの解析、TDDに基づいた実装、コードレビューまで順番にAIが自律的に行うように設計した。</br>
...
<br>→音声アップロード機能を導入し、会議で録音した音声データを取り込み、議事録作成・要約・アクション抽出まで一気通貫で行えるようにする。</br>
